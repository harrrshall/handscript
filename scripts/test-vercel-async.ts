...
const TEMP_IMG_DIR = path.join(process.cwd(), 'scripts', 'temp_images_async_test');

async function main() {
    console.log('Starting Vercel Async E2E Test...');
    ...
    for (const file of files) {
        ...
        const key = `inputs/${Date.now()}-${Math.random().toString(36).substring(7)}-${file}`;
