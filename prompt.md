# HandScript Master Transcription Prompt

**Drop-in replacement for `BATCH_SYSTEM_PROMPT` in `lib/gemini.ts`**
...
## Usage in `lib/gemini.ts`

Replace your current `BATCH_SYSTEM_PROMPT` with the prompt above (everything between the triple backticks).

```typescript
export const BATCH_SYSTEM_PROMPT = `
[paste entire prompt above here]
`;
````

The prompt is self-contained and requires no additional context injection or multi-pass processing.
