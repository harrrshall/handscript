...
| ------- | ---------------------------- | ------------ | ------------------------ | --------------------------------------------------------------- |
| ENV-001 | 🔴 Required vars present     | Critical     | [LOCAL] [STAGING] [PROD] | Verify all required environment variables are set and non-empty |
| ENV-002 | 🔴 GEMINI_API_KEY validity   | Critical     | [LOCAL] [STAGING]        | Make a simple Gemini API call to verify key works               |
| ENV-003 | 🔴 Redis connection test     | Critical     | [LOCAL] [STAGING] [PROD] | Verify UPSTASH_REDIS_REST_URL and TOKEN allow connection        |
| ENV-004 | 🔴 B2 credentials test       | Critical     | [LOCAL] [STAGING]        | Upload and delete a small test file to verify B2 access         |
| ENV-005 | 🟡 QStash token validity     | Important    | [STAGING] [PROD]         | Publish a test message to verify QSTASH_TOKEN works             |
| ENV-006 | 🟡 Resend API key test       | Important    | [LOCAL] [STAGING]        | Send a test email to verify RESEND_API_KEY                      |
| ENV-007 | 🟡 Modal endpoint reachable  | Important    | [STAGING] [PROD]         | HTTP GET/OPTIONS to MODAL_PDF_ENDPOINT returns 200/405          |
| ENV-008 | 🟢 VERCEL_URL resolution     | Nice-to-have | [STAGING] [PROD]         | Verify VERCEL_URL points to correct deployment                  |
| ENV-009 | 🔴 Build succeeds with env   | Critical     | [LOCAL]                  | `npm run build` completes without environment errors            |
| ENV-010 | 🟡 .env.example completeness | Important    | [LOCAL]                  | All vars in code have corresponding .env.example entry          |

**Test Procedures:**

**ENV-001: Required vars present**

```
Preconditions: Application environment loaded
Steps:
1. Check GEMINI_API_KEY is defined and length > 20
2. Check UPSTASH_REDIS_REST_URL starts with "https://"
3. Check UPSTASH_REDIS_REST_TOKEN is defined
4. Check B2_ENDPOINT, B2_REGION, B2_KEY_ID, B2_APPLICATION_KEY, B2_BUCKET_NAME all defined
Expected: All checks pass
Failure: Log which variables are missing/invalid
```

**ENV-003: Redis connection test**

```
Preconditions: Redis credentials configured
Steps:
1. Call redis.set("test:connection", "ok", { ex: 10 })
2. Call redis.get("test:connection")
3. Call redis.del("test:connection")
Expected: All operations succeed, get returns "ok"
Failure: Connection timeout or auth error
```
...
| ------- | ------------------------------------------------- | ------------ | ---------------------------------------------------------------- |
| GEM-001 | 🔴 generateBatchNotes returns valid BatchResponse | Critical     | Given valid image URLs, returns parsed response matching schema  |
| GEM-002 | 🔴 Schema cleaning removes additionalProperties   | Critical     | cleanSchema function removes all additionalProperties keys       |
| GEM-003 | 🔴 Invalid JSON response throws                   | Critical     | When Gemini returns non-JSON, function throws descriptive error  |
| GEM-004 | 🔴 Schema validation failure throws               | Critical     | When response doesn't match BatchResponseSchema, throws ZodError |
| GEM-005 | 🟡 Empty URL array handled                        | Important    | Calling with [] returns valid empty response or throws           |
| GEM-006 | 🟡 Single image processing                        | Important    | Works correctly with just 1 image                                |
| GEM-007 | 🟡 Maximum images (5) processing                  | Important    | Works correctly with 5 images (batch size)                       |
| GEM-008 | 🟢 URL with special characters                    | Nice-to-have | Signed URLs with encoded chars processed correctly               |
| GEM-009 | 🔴 Expired URL error detection                    | Critical     | When URL expired, error message indicates URL issue              |
| GEM-010 | 🟡 Rate limit error detection                     | Important    | Gemini 429 response is identified and logged                     |

**Test Procedures:**

**GEM-001: generateBatchNotes returns valid BatchResponse**

```
Preconditions: Valid GEMINI_API_KEY, test image uploaded to B2
Steps:
1. Generate signed URL for test image
2. Call generateBatchNotes([signedUrl])
3. Validate response has metadata.title, metadata.subject, metadata.documentType
4. Validate response.pages is array with at least 1 page
5. Validate pages[0].pageIndex === 0
6. Validate pages[0].content is array
Expected: All validations pass, response matches BatchResponseSchema
```

**GEM-004: Schema validation failure throws**

```
Preconditions: Mock Gemini to return malformed response
Steps:
1. Override generateContent to return { invalid: "data" }
2. Call generateBatchNotes([...])
Expected: ZodError thrown with path to invalid field
```

### 2.2 lib/formatting.ts

| ID      | Test Name                           | Priority     | Description                                                  |
| ------- | ----------------------------------- | ------------ | ------------------------------------------------------------ |
| FMT-001 | 🔴 renderToHtml paragraph block     | Critical     | Paragraph block renders to `<p>` tag                         |
| FMT-002 | 🔴 renderToHtml heading levels      | Critical     | Headings 1-6 render correctly with h1-h6 tags                |
| FMT-003 | 🔴 renderToHtml math display mode   | Critical     | Display math renders with katex-display class                |
| FMT-004 | 🔴 renderToHtml math inline mode    | Critical     | Inline math renders without display wrapper                  |
| FMT-005 | 🔴 renderToHtml ordered list        | Critical     | Ordered list renders as `<ol>` with `<li>` items             |
| FMT-006 | 🔴 renderToHtml unordered list      | Critical     | Unordered list renders as `<ul>` with `<li>` items           |
| FMT-007 | 🔴 renderToHtml container types     | Critical     | Each container kind (theorem, proof, etc.) has correct class |
| FMT-008 | 🔴 renderToHtml diagram block       | Critical     | Diagram renders as figure with description                   |
| FMT-009 | 🟡 sanitizeLatex removes delimiters | Important    | $$ and $ delimiters stripped from latex string               |
| FMT-010 | 🟡 processContent inline math       | Important    | $x^2$ in text becomes inline KaTeX                           |
| FMT-011 | 🟡 processContent display math      | Important    | $$x^2$$ in text becomes display KaTeX                        |
| FMT-012 | 🟡 processContent bold text         | Important    | **text** becomes `<strong>`                                  |
| FMT-013 | 🟡 processContent italic text       | Important    | _text_ becomes `<em>`                                        |
| FMT-014 | 🔴 KaTeX error handling             | Critical     | Invalid LaTeX shows error message, doesn't crash             |
| FMT-015 | 🟡 Table rendering                  | Important    | Markdown tables render as HTML tables                        |
| FMT-016 | 🟡 Table alignment                  | Important    | :---: center, ---: right alignment applied                   |
| FMT-017 | 🟢 Nested math in container         | Nice-to-have | Container with math content renders both correctly           |
| FMT-018 | 🟢 Empty content blocks             | Nice-to-have | Empty arrays/strings don't crash                             |

**Test Procedures:**

**FMT-001: renderToHtml paragraph block**

```
Input: { metadata: {...}, content: [{ type: "paragraph", text: "Hello world" }] }
Expected Output: Contains "<p>Hello world</p>"
```

**FMT-014: KaTeX error handling**

```
Input: { type: "math", latex: "\\invalidcommand{x}", display: true }
Expected: Output contains "katex-error" class, does not throw exception
```
...
| ID      | Test Name                                    | Priority  | Description                                      |
| ------- | -------------------------------------------- | --------- | ------------------------------------------------ |
| QUE-001 | 🔴 publishToQStash localhost bypass          | Critical  | localhost URLs don't call QStash, return mock ID |
| QUE-002 | 🔴 publishToQStash 127.0.0.1 bypass          | Critical  | 127.0.0.1 URLs bypass QStash                     |
| QUE-003 | 🔴 publishToQStash ::1 bypass                | Critical  | IPv6 loopback bypassed                           |
| QUE-004 | 🔴 publishToQStash production call           | Critical  | Non-localhost URLs call QStash.publishJSON       |
| QUE-005 | 🟡 Missing QSTASH_TOKEN handling             | Important | Returns "skipped-no-token" when token missing    |
| QUE-006 | 🔴 queueEmailDelivery constructs correct URL | Critical  | Uses VERCEL_URL or localhost for base            |
| QUE-007 | 🔴 queueErrorEmail constructs correct URL    | Critical  | Uses VERCEL_URL or localhost for base            |
| QUE-008 | 🟡 Localhost bypass makes fetch call         | Important | Fire-and-forget fetch is made to local URL       |

**Test Procedures:**

**QUE-001: publishToQStash localhost bypass**

```
Preconditions: Mock fetch, set QSTASH_TOKEN
Steps:
1. Call publishToQStash("http://localhost:3000/api/test", { data: 1 })
2. Verify QStash.publishJSON was NOT called
3. Verify local fetch was called
4. Verify return value has messageId: "local-dev-mock-id"
```
...
| ID     | Test Name                                  | Priority  | Description                                 |
| ------ | ------------------------------------------ | --------- | ------------------------------------------- |
| S3-001 | 🔴 uploadFile returns key                  | Critical  | Successful upload returns the key string    |
| S3-002 | 🔴 deleteFile single key                   | Critical  | Deletes single file by key                  |
| S3-003 | 🔴 deleteFile array of keys                | Critical  | Deletes multiple files                      |
| S3-004 | 🟡 deleteFile handles URLs                 | Important | Full URLs are parsed to extract key         |
| S3-005 | 🔴 getDownloadUrl generates signed URL     | Critical  | Returns URL with signature params           |
| S3-006 | 🔴 getUploadPresignedUrl generates PUT URL | Critical  | Returns URL usable for PUT                  |
| S3-007 | 🟡 listFiles pagination                    | Important | Returns blobs with cursor for pagination    |
| S3-008 | 🟡 cleanToken removes quotes               | Important | Tokens with quotes are cleaned              |
| S3-009 | 🔴 Endpoint formatting                     | Critical  | Endpoint without https:// prefix is handled |
| S3-010 | 🟡 Delete non-existent file                | Important | Doesn't throw, logs warning                 |

### 2.5 lib/schema.ts

| ID      | Test Name                              | Priority  | Description                           |
| ------- | -------------------------------------- | --------- | ------------------------------------- |
| SCH-001 | 🔴 ContentBlockSchema paragraph        | Critical  | Validates paragraph type correctly    |
| SCH-002 | 🔴 ContentBlockSchema heading          | Critical  | Validates heading with level 1-3      |
| SCH-003 | 🔴 ContentBlockSchema math             | Critical  | Validates math with latex and display |
| SCH-004 | 🔴 ContentBlockSchema list             | Critical  | Validates list with items array       |
| SCH-005 | 🔴 ContentBlockSchema container        | Critical  | Validates container with kind enum    |
| SCH-006 | 🔴 ContentBlockSchema diagram          | Critical  | Validates diagram with description    |
| SCH-007 | 🔴 BatchResponseSchema complete        | Critical  | Full response validates correctly     |
| SCH-008 | 🟡 Invalid type discriminator rejected | Important | Unknown type throws ZodError          |
| SCH-009 | 🟡 Heading level bounds                | Important | Level 0 and 4 are rejected            |
| SCH-010 | 🟡 Container kind enum                 | Important | Invalid kind value rejected           |
